    ...
    for iter in range(epoch):
        ...
        for batch_idx, (images, labels) in enumerate(train_dataloader):
            ...
            if with_softmax:
                ...
                # to do # 将logits labels的tensor类型转为longtensor，以便使用交叉熵作为损失函数
...
#
def get_models_logits(raw_logits, weight, N_models): #raw_logits为list-np；weight为tensor；
    loss_fn = torch.nn.MSELoss(reduce=True, size_average=True)
    teacher_logits = torch.zeros(N_models, np.size(raw_logits[0],0), np.size(raw_logits[0],1), requires_grad=True) #创建logits of teacher
    ...
    for self_idx in range(N_models): #对每个model计算其teacher的logits加权平均值
        ...
        loss_input = torch.autograd.Variable(torch.from_numpy(raw_logits[self_idx]))
        loss_target = torch.autograd.Variable(teacher_logits[self_idx])                                                          
